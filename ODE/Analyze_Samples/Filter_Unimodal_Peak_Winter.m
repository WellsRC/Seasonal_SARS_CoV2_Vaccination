...
p_N=min((0.66.*sum(Count_W)./sum(Count_WN))./3,1);% need monthly average
p_O=min((0.33*sum(Count_W)./sum(Count_WO))./3,1);% need monthly average

p_M=min((0.66.*sum(Count_W)./sum(Count_WM))./3,1);% need monthly average
p_A=min((0.33.*sum(Count_W)./sum(Count_WA))./3,1);% need monthly average

Weighted_Winter_Peak=zeros(size(Time_Peak));
...
RSW=rand(size(Pks));
Weighted_Selection= double((RSW<=Weighted_Winter_Peak));

Pks=Pks.*Weighted_Selection;
