...
Y=zeros(18,length(xbin_edges)-1);
...
for jj=1:3    
    for ss=1:6
        if(jj==1)
            ...
            Y(ss+6.*(jj-1),:)=Comparison.Histogram.Age_Cumulative_Count_Incidence_rel(ss,:)./MM;
            Per_Increase(ss+6.*(jj-1))=sum(Y(ss+6.*(jj-1),xt>=0))./sum(Y(ss+6.*(jj-1),:));
        elseif(jj==2)    
            MM=max(Comparison.Histogram.Age_Cumulative_Count_Hospital_rel(ss,:));
            Y(ss+6.*(jj-1),:)=Comparison.Histogram.Age_Cumulative_Count_Hospital_rel(ss,:)./MM;
            Per_Increase(ss+6.*(jj-1))=sum(Y(ss+6.*(jj-1),xt>=0))./sum(Y(ss+6.*(jj-1),:));
        elseif(jj==3)    
            MM=max(Comparison.Histogram.Age_Cumulative_Count_Death_rel(ss,:));
            Y(ss+6.*(jj-1),:)=Comparison.Histogram.Age_Cumulative_Count_Death_rel(ss,:)./MM;
            Per_Increase(ss+6.*(jj-1))=sum(Y(ss+6.*(jj-1),xt>=0))./sum(Y(ss+6.*(jj-1),:));
        end
        min_x=min(min_x,min(xbin_edges(Y(ss+6.*(jj-1),:)>0)));
        max_x=max(max_x,max(xbin_edges(Y(ss+6.*(jj-1),:)>0)));
    end
...
for ii=1:18
    patch(100.*([xt flip(xt)]),ii+[-0.45.*Y(ii,:) flip(0.45.*Y(ii,:))], CC(C_indx(ii),:),'LineStyle','none'); hold on; 
end
