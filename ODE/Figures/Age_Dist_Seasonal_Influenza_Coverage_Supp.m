function Age_Dist_Seasonal_Influenza_Coverage_Supp(Scenario,State_V)
close all;
...
for Age_Indx=1:6
    load([temp_cd 'Comparison_Summary_' Scenario '_Seasonal_Influenza_Coverage_Annual_Ages_' num2str(Age_B(Age_Indx,1)) '_to_' num2str(Age_B(Age_Indx,1)) '=' num2str(100.*vac_coverage_influenza(Age_Indx,1)) '.mat'],'xbin_edges');
    ...
    for mm=1:4
        ...
        load([temp_cd 'Comparison_Summary_' Scenario '_Seasonal_Influenza_Coverage_Annual_Ages_' num2str(Age_B(Age_Indx,1)) '_to_' num2str(Age_B(Age_Indx,1)) '=' num2str(100.*vac_coverage_influenza(Age_Indx,mm)) '.mat']);
        for ss=1:6
            if(t_out(1))
    ...
    for ss=1:6
        ...
        xlabel(['Vaccination coverage for those 0' char(8211) '4'],'Fontsize',16);
        text(-0.204,1.046,char(64+ss),'fontweight','bold','units','normalized','Fontsize',28);  
    end
    print(gcf,['Supplement_Seasonal_Influenza_Coverage_Annual_Ages_' num2str(Age_B(Age_Indx,1)) '_to_' num2str(Age_B(Age_Indx,2)) '_' Scenario '_' State_V '.png'],'-dpng','-r300');
end
end