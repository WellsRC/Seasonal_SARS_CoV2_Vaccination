function Age_Dist_Baseline_Coverage_Main(State_V)
close all;
Scenario='All';
temp_cd=pwd;
...
for Age_Indx=1:6
    load([temp_cd 'Comparison_Summary_' Scenario '_Baseline_Coverage_Annual_Ages_' num2str(Age_B(Age_Indx,1)) '_to_' num2str(Age_B(Age_Indx,2)) '=' num2str(100.*vac_coverage_baseline(Age_Indx,1)) '.mat'],'xbin_edges');
    ...
    XTL=cell(1,4);
    for mm=1:4
        XTL{mm}=[num2str(100.*vac_coverage_baseline(Age_Indx,mm)) '%'];
        load([temp_cd 'Comparison_Summary_' Scenario '_Baseline_Coverage_Annual_Ages_' num2str(Age_B(Age_Indx,1)) '_to_' num2str(Age_B(Age_Indx,2)) '=' num2str(100.*vac_coverage_baseline(Age_Indx,mm)) '.mat']);
...
xt=-xt;
    ...
    for jj=1:2
        ...
        if(jj==1)
            ...
            text(0.5,41.5,['Ages ' AgeC{Age_Indx}],'Fontsize',14,'color',CC(t_out,:));
            text(0.5,37.5,['Those not ' AgeC{Age_Indx}],'Fontsize',14,'color',interp1([0 1],[1 1 1; CC(t_out,:)],0.45));
        else
            for mm=1:4
                patch(mm+[-0.45.*Y(jj+1,:,mm) flip(0.*Y(jj+1,:,mm))], 100.*([xt flip(xt)]),CC(t_out,:),'LineStyle','none'); hold on; 
                patch(mm+[0.*Y(jj+2,:,mm) flip(0.45.*Y(jj+2,:,mm))], 100.*([xt flip(xt)]),interp1([0 1],[1 1 1; CC(t_out,:)],0.45),'LineStyle','none'); hold on; 
                text(mm,-7,['(' num2str(100.*Per_Increase(1,mm),'%3.1f') '%)'],'HorizontalAlignment','center','fontsize',16,'color',CC(t_out,:));
                text(mm,-12,['(' num2str(100.*Per_Increase(2,mm),'%3.1f') '%)'],'HorizontalAlignment','center','fontsize',16,'color',interp1([0 1],[1 1 1; CC(t_out,:)],0.45));
                text(0.5,41.5,['All ages'],'Fontsize',14,'color',CC(t_out,:));
                text(0.5,37.5,['Ages 65+'],'Fontsize',14,'color',interp1([0 1],[1 1 1; CC(t_out,:)],0.45));
            end
        end
        box off;
        set(gca,'LineWidth',2,'Tickdir','out','XTick',[1:4],'XTickLabel',XTL,'Yminortick','off','YTick',[-10:10:40],'Xminortick','off','Fontsize',16);
        ylim([-15 45])
        xlim([0.45 4.55])
    ...
    print(gcf,['Main_Baseline_Coverage_Annual_Ages_' num2str(Age_B(Age_Indx,1)) '_to_' num2str(Age_B(Age_Indx,2)) '.png'],'-dpng','-r300');
end
end