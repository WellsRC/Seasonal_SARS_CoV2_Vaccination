...
for Scenario_Indx=1:4    
    Y=zeros(3,length(xt),length(XTL));

    min_x=Inf;
    max_x=-Inf;
    M_R=zeros(length(XTL),3);
    for mm=1:length(XTL)
        ...
        for aav=1:3
            ...
            if(Scenario_Indx==1)
                ...
                Y(aav,:,mm)=pdf(Comparison.Histogram.Age_Cumulative_Count_Incidence_rel{aa},xt)./MM;
            elseif(Scenario_Indx==2)   
                MM=max(pdf(Comparison.Histogram.Age_Cumulative_Count_Hospital_rel{aa},xt));
                Y(aav,:,mm)=pdf(Comparison.Histogram.Age_Cumulative_Count_Hospital_rel{aa},xt)./MM;
            elseif(Scenario_Indx==3)   
                MM=max(pdf(Comparison.Histogram.Age_Cumulative_Count_Death_rel{aa},xt));
                Y(aav,:,mm)=pdf(Comparison.Histogram.Age_Cumulative_Count_Death_rel{aa},xt)./MM;
            elseif(Scenario_Indx==4)   
                MM=max(pdf(Comparison.Histogram.Cost_Age_rel{aa},xt));
                Y(aav,:,mm)=pdf(Comparison.Histogram.Cost_Age_rel{aa},xt)./MM;
                M_R(mm,:)=[Comparison.PRCT.Cost_Age_rel(PRCT==50,aa,end) Comparison.PRCT.Cost_Age_rel(PRCT==2.5,aa,end) Comparison.PRCT.Cost_Age_rel(PRCT==97.5,aa,end)];
            end
            min_x=min(min_x,min(xt(Y(aav,:,mm)>0)));
            max_x=max(max_x,max(xt(Y(aav,:,mm)>0)));
        end
    end
...
%     
        ...
        for mm=1:length(XTL)
            t1=squeeze(Y(aa,:,mm));
            patch(mm+[-0.45.*t1 flip(0.45*t1)], 100.*([xt2 flip(xt2)]),CC(Scenario_Indx,:),'LineStyle','none'); hold on; 
        end
        box off;
