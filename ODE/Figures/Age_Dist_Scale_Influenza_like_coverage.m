function Age_Dist_Scale_Influenza_like_coverage(Scenario,State_V)
close all;
...
load([temp_cd 'Comparison_Summary_' Scenario '-Low_Coverage_Annual_Booster_Baseline_scaled_influenza=20.mat'],'xbin_edges');
...
Red_VC=[1 0.2:0.2:0.8];
XTL={'0%','20%','40%','60%','80%'};
for mm=1:5
    load([temp_cd 'Comparison_Summary_' Scenario '-Low_Coverage_Annual_Booster_Baseline_scaled_influenza=' num2str(100.*Red_VC(mm)) '.mat']);
    for ss=1:6
        if(ss<6)
            if(t_out(1))
...
%         text(mm,25,['(' num2str(100.*Per_Increase(ss,mm),'%3.1f') '%)'],'HorizontalAlignment','center','fontsize',12);
    ...
    ylabel('Relative to baseline coverage','Fontsize',18);
    xlabel('Relative reduction from influenza-like','Fontsize',16);
end
end