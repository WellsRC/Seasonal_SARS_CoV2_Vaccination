function [T_Run,Y_Model] = Run_Annual_Booster_ODE(T_Run,Parameters)

opts = odeset('RelTol',1e-8,'AbsTol',1e-8,'NonNegative',1:length(Parameters.X0.Annual));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[T,Y] = ode45(@(t,x)Annual_SEIRVS(t,x,Parameters.beta_I,Parameters.sigma_E,Parameters.delta_I,Parameters.omega_R1,Parameters.omega_R2,Parameters.omega_R3,Parameters.omega_R4,Parameters.omega_R5,Parameters.alpha_R1,Parameters.alpha_R2,Parameters.alpha_R4,Parameters.p1_inf,Parameters.p2_inf,Parameters.p3_inf,Parameters.p1_sd,Parameters.p2_sd,Parameters.eps_V1,Parameters.eps_V2,Parameters.eps_V3,Parameters.q1_sd,Parameters.q2_sd,Parameters.q3_sd,Parameters.gammaV_1,Parameters.gammaV_2,Parameters.gammaV_3,Parameters.kappaV_1,Parameters.kappaV_2,Parameters.delta_V,Parameters.psi_V,Parameters.nu_V_Influenza,Parameters.C,Parameters.prob_H,pd,Parameters.Delay_Time), [min(T_Run) max(T_Run)+2],Parameters.X0.Annual, opts);
A=length(pd);
C_Incidence=Y(:,28+30.*[0:(A-1)]);
C_Death=Y(:,29+30.*[0:(A-1)]);
C_Hosp=Y(:,30+30.*[0:(A-1)]);

SD_Natural=zeros(size(Y(:,13+30.*[0:(A-1)])));
I_Natural=zeros(size(Y(:,13+30.*[0:(A-1)])));
for jj=13:27
    SD_Natural=SD_Natural+Y(:,jj+30.*[0:(A-1)]);
    if((jj>=13 && jj<=15)||(jj>=18 && jj<=20)||(jj>=23 && jj<=25))
        I_Natural=I_Natural+Y(:,jj+30.*[0:(A-1)]);
    end
end
SD_Vaccine=zeros(size(Y(:,16+30.*[0:(A-1)])));
I_Vaccine=zeros(size(Y(:,16+30.*[0:(A-1)])));
for jj=4:6
    SD_Vaccine=SD_Vaccine+Y(:,jj+30.*[0:(A-1)]);
    switch jj
        case 4
            I_Vaccine=I_Vaccine+repmat(Parameters.eps_V1',length(T),1).*Y(:,jj+30.*[0:(A-1)]);
        case 5
            I_Vaccine=I_Vaccine+repmat(Parameters.eps_V2',length(T),1).*Y(:,jj+30.*[0:(A-1)]);
        case 6
            I_Vaccine=I_Vaccine+repmat(Parameters.eps_V3',length(T),1).*Y(:,jj+30.*[0:(A-1)]);
    end
end
F_Susceptible=Y(:,1+30.*[0:(A-1)])+Y(:,2+30.*[0:(A-1)])+Y(:,3+30.*[0:(A-1)]);
